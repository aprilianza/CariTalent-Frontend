...
export const createApiClient = () => {
   ...
   */
  const request = async <T>(endpoint: string, options: ApiClientOptions = {}): Promise<ApiResponse<T>> => {
    const { method = 'GET', body, query, headers = {}, requiresAuth = true } = options;
    ...
    try {
      ...
      if (error.statusCode === 422) {
        throw new ApiError(error.data?.message || 'Validation failed', 422, error.data?.errors);
      }
  ...
  return {
     ...
     * Makes a GET request
     */
    get: <T>(endpoint: string, query?: Record<string, any>, requiresAuth = true) => request<T>(endpoint, { method: 'GET', query, requiresAuth }),

    /**
     * Makes a POST request
     */
    post: <T>(endpoint: string, body?: any, requiresAuth = true) => request<T>(endpoint, { method: 'POST', body, requiresAuth }),

    /**
     * Makes a PUT request
     */
    put: <T>(endpoint: string, body?: any, requiresAuth = true) => request<T>(endpoint, { method: 'PUT', body, requiresAuth }),
