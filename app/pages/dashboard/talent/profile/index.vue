<template>
  <div class="space-y-6">
    <UiCard card-class="overflow-hidden border-white/10 bg-gradient-to-br from-highlight/12 via-white/5 to-accent/12 shadow-md">
      <div class="space-y-3">
          ...
          <h1 class="font-display text-2xl font-bold bg-gradient-to-r from-highlight to-accent bg-clip-text text-transparent md:text-3xl">Pengaturan Profil</h1>
      ...
      <UiCard>
        <template #header>
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="space-y-1">
              <h3 class="text-base font-semibold text-ui-light">Ringkasan Profil</h3>
        ...
        <form v-else-if="isEditingOverview" class="space-y-5" @submit.prevent="handleUpdateOverview">
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField label="Nama" required class="w-full">
            ...
            <UFormField label="Telepon" required class="w-full">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField label="Nama Panggung" required class="w-full">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField label="Harga Min" class="w-full">
            ...
            <UFormField label="Harga Max" class="w-full">
          ...
          <UFormField label="Link Portofolio" class="w-full">
        ...
        <div v-else class="space-y-5">
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Nama</p>
              <p class="mt-1 text-base font-semibold text-ui-light">{{ profile.name || '-' }}</p>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Telepon</p>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Nama Panggung</p>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Kota</p>
              <p class="mt-1 text-base font-semibold text-ui-light">{{ profile.city || '-' }}</p>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Portofolio</p>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Harga Min</p>
              <p class="mt-1 text-base font-semibold text-ui-light">{{ profile.price_min ? formatCurrency(profile.price_min) : '-' }}</p>
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Harga Max</p>
              <p class="mt-1 text-base font-semibold text-ui-light">{{ profile.price_max ? formatCurrency(profile.price_max) : '-' }}</p>
            </div>
          </div>

          <div>
            <p class="text-xs uppercase tracking-wide text-neutral-light/70">Genre</p>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Verifikasi</p>
              <UiBadge class="mt-2" :label="profile.verified ? 'Terverifikasi' : 'Belum Verifikasi'" :color="profile.verified ? 'success' : 'warning'" variant="soft" />
            </div>
            <div class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-wide text-neutral-light/70">Rating Rata-rata</p>
      ...
      <section class="grid gap-5">
        <UiCard title="Ubah Password" description="Ubah password akunmu dengan aman." card-class="h-full">
          <form class="flex h-full flex-col gap-5" @submit.prevent="handleChangePassword">
            <div class="grid gap-4">
              <UFormField label="Password Saat Ini" required class="w-full">
              ...
              <UFormField label="Password Baru" required class="w-full">
              ...
              <UFormField label="Konfirmasi Password Baru" required class="w-full">
            ...
            <UiButton type="submit" color="secondary" block class="mt-auto" :loading="isChangingPassword">Perbarui Password</UiButton>
          </form>
        </UiCard>
      </section>
    </div>
  ...
  useState('talent-layout-title', () => 'Dashboard Talent').value = 'Profil';
